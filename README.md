...
Теперь пошагово объясним, как работает пирамидальная сортировка, используя массив [5, 3, 8, 1, 4]. Мы начнем с создания максимальной кучи из массива.
...
/ \  
1  4  
Начинаем с среднего элемента (индекс = 5 // 2 = 2). Элемент 8 уже является наибольшим среди своих детей.  
Переходим к элементу 3 (индекс 1). 5 больше 3, меняем местами:  
   5  
  / \  
 8   3  
/ \  
1  4  
Переходим к элементу 5 (индекс 0). 8 больше 5, меняем местами:  
   8  
  / \  
 5   3  
/ \  
1  4   
Теперь у нас есть максимальная куча: [8, 5, 3, 1, 4].  

2. Теперь начинаем сортировку с помощью пирамидальной сортировки.
Алгоритм меняет местами самый большой элемент (корень кучи) с последним элементом массива. Теперь самый большой элемент находится на конечной позиции массива. После этого алгоритм уменьшает размер кучи на один (исключает последний элемент из кучи) и восстанавливает свойства максимальной кучи, "просеивая" новый корень кучи вниз. Процесс повторяется, пока в куче остается больше одного элемента. В результате получаем отсортированный массив.

Меняем местами корень (8) с последним элементом (4):  
   4  
  / \  
 5   3  
/  
1  
Восстанавливаем свойства максимальной кучи. 5 больше 4, меняем местами:  
   5  
  / \  
 4   3  
/  
1  
Меняем местами корень (5) с предпоследним элементом (1):  
   1  
  / \  
 4   3  
/ \  
5  8  
Восстанавливаем свойства максимальной кучи. 4 больше 1, меняем местами:  
   4  
  / \  
 1   3  
/ \  
5  8  
Меняем местами корень (4) и последний неотсортированный элемент (3):  
   3  
  / \  
 1   4  
/ \  
5  8  
Восстанавливаем свойства максимальной кучи. 3 уже больше своих детей.  
Все элементы отсортированы: [1, 3, 4, 5, 8].  
